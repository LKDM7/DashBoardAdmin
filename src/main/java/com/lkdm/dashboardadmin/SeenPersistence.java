...
public class SeenPersistence {
    private static final Path PATH = Paths.get("run/data/last_seen.json");
    private static final Gson GSON = new GsonBuilder().setPrettyPrinting().create();

    public static void save() {
        JsonObject data = new JsonObject();
        for (var e : DashboardAdmin.getLastSeenTimestamps().entrySet()) {
    ...
    public static void load() {
        ...
        try {
            ...
            Map<UUID, Long> map = DashboardAdmin.getLastSeenTimestamps();
            for (var e : json.entrySet()) {
                map.put(UUID.fromString(e.getKey()), e.getValue().getAsLong());
            }
        } catch (IOException e) { e.printStackTrace(); }
    }
